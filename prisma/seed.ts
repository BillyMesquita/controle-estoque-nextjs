...
async function main() {
  ...
  await prisma.user.createMany({
    data: [
      { name: 'Admin', email: 'admin@sistema.com', passwordHash: bcrypt.hashSync('admin123', 10), role: UserRole.Administrador },
      { name: 'Operador', email: 'operador@sistema.com', passwordHash: bcrypt.hashSync('operador123', 10), role: UserRole.Operador },
    ],
  })
