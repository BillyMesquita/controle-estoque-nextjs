      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {form.type !== 'Saida' && <div><label className="label">Destino</label><input className="input-field" value={form.destino} onChange={set('destino')} placeholder="Opcional" /></div>}
            {form.type !== 'Saida' && <div><label className="label">Evento {form.type !== 'Entrada' && <span className="text-red-500">*</span>}</label><select className="input-field" value={form.eventId} onChange={set('eventId')}>{form.type === 'Entrada' && <option value="">Nenhum</option>}{[...activeEvents, ...finishedEvents].map((ev: any) => <option key={ev.id} value={ev.id}>{ev.name}</option>)}</select></div>}
          </div>
